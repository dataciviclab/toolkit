...
from toolkit.domain.readiness import summary as _summary
...
def _print_layer_profiles(dataset: str, year: int, layers: dict[str, Any]) -> None:
    ...
    mart_tables: list[dict[str, Any]] = mart_tables_raw if isinstance(mart_tables_raw, list) else []
    if mart_tables:
    ...
    transitions: list[dict[str, Any]] = transitions_raw if isinstance(transitions_raw, list) else []
    if transitions:
...
def summary(
    ...
    Esempi:
        ...
        toolkit inspect summary -c dataset.yml --json
        toolkit inspect summary -c dataset.yml --run-id <id>
    """
