...
def _scaffold_sdmx(
    ...
    *,
    ...
    flow = sdmx_info.get("flow_id", "?")
    version = sdmx_info.get("version")
    ...
    # clean.sql con dimensioni scoperte
    clean_sql_path = out_dir / "sql" / "clean.sql"
    clean_sql_path.parent.mkdir(parents=True, exist_ok=True)
    clean_lines = [
        f"-- SDMX flow: {flow}" + (f" (version {version})" if version else ""),
        "-- Le colonne _label contengono la descrizione testuale dei codici.",
        "-- Personalizza: seleziona solo le dimensioni e misure che servono.",
        "SELECT",
        "  *",
    ]
    for dim in dimensions:
        if dim == "TIME_PERIOD":
            continue
        clean_lines.append(f"  , {dim}_label")
    clean_lines += [
        "FROM raw_input",
        "",
    ]
    ...
    (out_dir / "notes.md").write_text(
        ...
        f"- Flow: {flow}\n"
        + (f"- Versione: {version}\n" if version else "")
        + (f"- Dimensioni: {', '.join(dimensions[:8])}\n" if dimensions else ""),
        encoding="utf-8",
    )
    (out_dir / "notebooks").mkdir(exist_ok=True)
    ...
    typer.echo(f"  flow: {flow}")
    if version:
        typer.echo(f"  version: {version}")
    if dimensions:
        typer.echo(f"  dimensions: {len(dimensions)} trovate")
    if year_min and year_max:
