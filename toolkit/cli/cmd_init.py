...
def _slugify(url: str) -> str:
    ...
    stem = Path(urlparse(url).path).stem or "dataset"
...
def _scout(url: str, *, timeout: int = 60) -> None:
    ...
    tmp_dir = Path("/tmp")
    sample_path = tmp_dir / f"scout_{slug}_{uuid.uuid4().hex[:8]}{ext}"
    ...
    if not result.is_ok or result.response is None:
        err = result.err
        raise typer.Exit(code=1, message=f"Failed to fetch {url}: {err}")

    content = result.response.content[:_SAMPLE_SIZE]
    ...
    if sample_path.suffix.lower() in (".xlsx", ".xls"):
        # Per Excel sniff non serve, profile direttamente
...
def _generate_dataset_yml(
    ...
    fname = Path(urlparse(url).path).name or f"{slug}.csv"
