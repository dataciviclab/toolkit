...
def _scout(url: str, *, timeout: int = 60) -> None:
    ...
    out_path = Path(f"{slug}/dataset.yml")
    out_path.parent.mkdir(parents=True, exist_ok=True)
    out_path.write_text(dataset_yml, encoding="utf-8")

    typer.echo(f"\nDataset YAML generated: {out_path}")
    typer.echo(f"Next: toolkit run raw --config {out_path}")
...
def init(
    ...
    Con --url scarica un sample, profila encoding/delim/colonne e genera
    dataset.yml completo, pronto per toolkit run raw.
