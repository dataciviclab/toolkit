...
def _scout(url: str, *, timeout: int = 60) -> None:
    ...
    # Check content-type to avoid HTML detected as CSV
    ct = (resp.headers.get("Content-Type") or "").lower()
    if "html" in ct:
        typer.echo(f"warning: URL appears to return HTML (Content-Type: {ct}), not a data file", err=True)
    ...
    # Detect ragged CSV and add strict_mode: false
    warnings = sniff_hints.get("warnings") or []
    if any("ragged" in str(w).lower() or "column count" in str(w).lower() or "mismatch" in str(w).lower() for w in warnings):
        read_cfg["strict_mode"] = False
        typer.echo("  Note: ragged CSV detected, strict_mode=false added")
