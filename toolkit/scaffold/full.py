...
def suggest_validation(profile: dict[str, Any]) -> dict[str, Any]:
    ...
    if norm_cols:
        # required_columns va in snake_case (come la clean output)
...
def generate_full_scaffold(
    ...
    *,
    ...
    # Nome SQL-safe per dataset.name e mart.tables[].name:
    # lo slug (directory) usa trattini, ma SQL/YAML require underscore.
    ...
    # required_columns a livello clean: (fuori da validate:)
    if validation_suggestions:
        cv = validation_suggestions.get("clean", {})
        req = cv.get("required_columns") or cv.get("validate", {}).get("required_columns")
        if req:
            items = ", ".join(f'"{c}"' for c in req)
            yml_lines.append(f"  required_columns: [{items}]")
    yml_lines.append('  sql: "sql/clean.sql"')

    if validation_suggestions:
        cv = validation_suggestions.get("clean", {})
        vblock = cv.get("validate", {})
        # required_columns non va dentro validate: — gia' emesso sopra.
        vblock.pop("required_columns", None)
        if vblock:
            yml_lines.append("  validate:")
            for key, val in vblock.items():
    ...
    yml_lines.append(f'    - name: "{safe_name}"')
