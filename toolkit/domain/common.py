"""Utility di dominio condivise — anni, profili, preview.

Estratto da ``toolkit.cli.common`` per eliminare dipendenze CLI.
Funzioni di pura logica di dominio.
"""
...
def load_layer_profile_summaries(root: Path, dataset: str, year: int) -> dict[str, object] | None:
    clean_metadata = (
        read_json_or_none(layer_year_dir(root, "clean", dataset, year) / METADATA) or {}
    )
    mart_metadata = read_json_or_none(layer_year_dir(root, "mart", dataset, year) / METADATA) or {}
